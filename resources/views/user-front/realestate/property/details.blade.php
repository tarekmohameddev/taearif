...
<meta property="og:url" content="{{ route('front.user.property.details', [getParam(), 'slug' => $propertyContent->slug]) }}">
@endsection
@else
                            ...
                            <div class="swiper-slide">
                                <figure class="radius-lg lazy-container ratio ratio-16-11">
                                    <a href="{{ asset('assets/img/property/slider-images/' . $slider->image) }}" class="lightbox-single">
                                        <img class="lazyload" data-src="{{ asset('assets/img/property/slider-images/' . $slider->image) }}" src="{{ asset('assets/img/property/slider-images/' . $slider->image) }}">
                                    </a>
                                </figure>
                            </div>
                            @endforeach
                ...
                <div class="product-single-details">
                    <div class="row">
                        <div class="col-md-8">
                            <div class="d-flex align-items-center justify-content-between mb-10">
                                <span class="product-category text-sm"> <a href="{{ route('front.user.properties', [getParam(), 'category' => $propertyContent->slug]) }}">
                        ...
                        <div class="col-md-4">
                                ...
                                <div class="user mb-20">
                                    <div class="user-img">
                                        <div class="lazy-container ratio ratio-1-1 rounded-pill">
                                            <img class="lazyload" src="{{ $user->photo ? asset('assets/front/img/user/' . $user->photo) : asset('assets/img/blank-user.jpg') }} " data-src=" {{ $user->photo ? asset('assets/front/img/user/' . $user->photo) : asset('assets/img/blank-user.jpg') }} ">
                            ...
                            <ul class="share-link list-unstyled mb-30">
                                ...
                                <li>
                                    ...
                                    @endphp
                                    @endif
                                    <a href="{{ route('front.user.property.add-to-wishlist', [getParam(), 'id' => $propertyContent->propertyId]) }}" class="btn red " data-tooltip="tooltip" data-bs-placement="top" title="{{ $checkWishList == false ? __('Add to Wishlist') : __('Saved') }}">
                    ...
                    <div class="row" class="mb-20">
                        ...
                        <div class="col-lg-3 col-sm-6 col-md-4 mb-20">
                            <strong class="mb-1 @if ($userBs->theme != 'home_five') text-dark @endif d-block">{{ $specification?->label }}</strong>
                    ...
                    <div class="product-video mb-40">
                        ...
                        <div class="lazy-container radius-lg ratio ratio-16-11">
                            <img class="lazyload" data-src="{{ $propertyContent->video_image ? asset('assets/img/property/video/' . $propertyContent->video_image) : asset('assets/front/images/placeholder.png') }}" src="{{ $propertyContent->video_image ? asset('assets/img/property/video/' . $propertyContent->video_image) : asset('assets/front/images/placeholder.png') }}">
                    ...
                    <div class="product-planning mb-40">
                        ...
                        <div class="lazy-container radius-lg ratio ratio-16-11 border">
                            <img class="lazyload" src="{{ asset('assets/img/property/plannings/' . $propertyContent->floor_planning_image) }}" data-src="{{ asset('assets/img/property/plannings/' . $propertyContent->floor_planning_image) }}">
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="product-location mb-40">
                        ...
                        <div class="lazy-container radius-lg ratio ratio-21-9 border">
                            <iframe class="lazyload" src="https://maps.google.com/maps?q={{ $propertyContent->latitude }},{{ $propertyContent->longitude }}&hl={{ $userCurrentLang->code }}&z=14&amp;output=embed"></iframe>
                        </div>
                    </div>
                    @endif
                    {{-- @if (!empty(showAd(3)))
                    ...
                    <aside class="sidebar-widget-area mb-10" data-aos="fade-up">
                        ...
                        <form action="{{ route('front.user.property_contact', getParam()) }}" method="POST">
                            ...
                            <div class="form-group mb-20">
                                <input type="text" class="form-control" name="name" placeholder="{{ $keywords['Name'] ?? __('Name') }}*" required value="{{ old('name') }}">
                                @error('name')
                                <p class=" text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="form-group mb-20">
                                <input type="email" class="form-control" required name="email" placeholder="{{ $keywords['Email Address'] ?? __('Email Address') }}*" value="{{ old('email') }}">
                                @error('email')
                                <p class=" text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="form-group mb-20">
                                <input type="number" class="form-control" name="phone" required value="{{ old('phone') }}" placeholder="{{ $keywords['Phone Number'] ?? __('Phone Number') }}*">
                                @error('phone')
                                <p class=" text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="form-group mb-20">
                                <textarea name="message" id="message" class="form-control" cols="30" rows="8" required="" data-error="Please enter your message" placeholder="{{ $keywords['Message'] ?? __('Message') }}...">{{ old('message') }}</textarea>
                    ...
                    <div class="accordion-body p-0">
                        ...
                        <div class="product-default product-inline mt-20">
                            <figure class="product-img">
                                <a href="{{ route('front.user.property.details', [getParam(), 'slug' => $property->slug]) }}" class="lazy-container ratio ratio-1-1 radius-md">
                                    <img class="lazyload" data-src="{{ asset('assets/img/property/featureds/' . $property->featured_image) }}" src="{{ asset('assets/img/property/featureds/' . $property->featured_image) }}">
                                </a>
                            </figure>
                            <div class="product-details">
                                <h6 class="product-title"><a href="{{ route('front.user.property.details', [getParam(), 'slug' => $property->slug]) }}">{{ $property->title }}</a>
                                </h6>
                        ...
                        <div class="widget widget-recent radius-md mb-30 ">
                            ...
                            <div id="products" class="collapse show">
                                ...
                                <ul class="product-info p-0 list-unstyled d-flex align-items-center">
                                    ...
                                    @if ($property->type == 'residential')
                                    <li class="icon-start" data-tooltip="tooltip" data-bs-placement="top" title="{{ $keywords['Bed'] ?? __('Bed') }}">
                                        <i class="fal fa-bed"></i>
                                        <span>{{ $property->beds }} </span>
                                    </li>
                                    <li class="icon-start" data-tooltip="tooltip" data-bs-placement="top" title="{{ $keywords['Bath'] ?? __('Bath') }}">
                                        <i class="fal fa-bath"></i>
                                        <span>{{ $property->bath }} </span>
                                    </li>
                                    @endif
