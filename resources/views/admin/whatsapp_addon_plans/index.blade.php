...
<div class="page-header">
    <h4 class="page-title">{{ __('WhatsApp Addon Plans') }}</h4>
    <ul class="breadcrumbs">
        <li class="nav-home"><a href="{{ route('admin.dashboard') }}"><i class="flaticon-home"></i></a></li>
        <li class="separator"><i class="flaticon-right-arrow"></i></li>
        <li class="nav-item"><a href="#">{{ __('Credit Management') }}</a></li>
        <li class="separator"><i class="flaticon-right-arrow"></i></li>
        <li class="nav-item"><a href="#">{{ __('WhatsApp Addon Plans') }}</a></li>
    </ul>
    <div class="ml-auto">
        <form class="form-inline" method="GET" action="{{ route('admin.whatsapp-addon-plans.index') }}">
            ...
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <p class="text-muted small mb-0">{{ __('Price and duration for WhatsApp number add-ons.') }}</p>
                    </div>
                    <div class="col-auto">
                ...
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4">{{ __('No WhatsApp addon plans available.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.whatsapp-addon-plans.store') }}">
                @csrf
...
<script>
    ...
    function editPlan(id) {
        ...
        $('#editForm').attr('action', '{{ route("admin.whatsapp-addon-plans.update", ":id") }}'.replace(':id', id));
    ...
    function toggleStatus(id) {
        ...
        $.ajax({
            url: '{{ route("admin.whatsapp-addon-plans.toggle-status", ":id") }}'.replace(':id', id),
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
    ...
    function deletePlan(id) {
        ...
        $.ajax({
            url: '{{ route("admin.whatsapp-addon-plans.destroy", ":id") }}'.replace(':id', id),
            method: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}'
            },
