...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="card">
        ...
        <div class="card-body">
          <form id="ajaxForm" action="{{ route('admin.support_center.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <label for="name">{{ __('Name') }} **</label>
              <input type="text" class="form-control" id="name" name="name" value="{{ $category->name }}" placeholder="{{ __('Enter category name') }}" required>
              <p id="errname" class="mb-0 text-danger em"></p>
            </div>
            <div class="form-group">
              <label for="slug">{{ __('Slug') }}</label>
              <input type="text" class="form-control" id="slug" name="slug" value="{{ $category->slug }}" placeholder="{{ __('Auto-generated from name') }}">
            ...
            <div class="form-group">
              ...
              <textarea class="form-control" id="short_description" name="short_description" rows="4" placeholder="{{ __('Enter short description') }}">{{ $category->short_description }}</textarea>
            ...
            <div class="form-group">
              ...
              @if ($category->icon_image)
                <div class="mt-2">
                  <img src="{{ asset($category->icon_image) }}" alt="{{ $category->name }}" style="max-width: 80px; max-height: 80px;">
                </div>
              @endif
              <p class="text-muted"><small>{{ __('JPG, PNG or WEBP. Max 5MB. Leave empty to keep current.') }}</small></p>
            </div>
          </form>
        </div>
  ...
  <script>
    document.getElementById('name').addEventListener('input', function() {
      if (!document.getElementById('slug').value || document.getElementById('slug').value === '{{ $category->slug }}') generateSlug(this.value);
    });
