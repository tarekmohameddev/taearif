        ...
        <div class="card-body">
          ...
          @if($article->cta_enabled)
            <div class="mt-4 p-3 bg-light">
              ...
              <a href="{{$article->cta_url}}" class="btn btn-primary" {{$article->cta_target_blank ? 'target="_blank"' : ''}}>
                {{$article->cta_text}}
              </a>
            </div>
          @endif
          <div class="mt-4">
            ...
            <hr>
            <p><strong>{{__('Meta Title')}}:</strong> {{$article->meta_title ?: __('Not set')}}</p>
            <p><strong>{{__('Meta Description')}}:</strong> {{$article->meta_description ?: __('Not set')}}</p>
            @if($article->og_image)
              <p><strong>{{__('OG Image')}}:</strong> <img src="{{asset($article->og_image)}}" style="max-width: 200px;"></p>
            @endif
          </div>
        </div>
      </div>
