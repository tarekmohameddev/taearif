...
@section('content')
  <div class="page-header">
    <h4 class="page-title">{{__('Admin Articles')}}</h4>
      ...
      <div class="card">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-8">
                    <form method="GET" action="{{route('admin.articles.index')}}" class="d-inline-flex">
                        ...
                        <input type="text" name="search" class="form-control mr-2" placeholder="{{__('Search')}}" value="{{request('search')}}">
                        <button type="submit" class="btn btn-primary btn-sm">{{__('Filter')}}</button>
                        <a href="{{route('admin.articles.index')}}" class="btn btn-secondary btn-sm ml-2">{{__('Reset')}}</a>
                    </form>
                </div>
                <div class="col-lg-12 mt-3">
                    <a href="{{route('admin.articles.create')}}" class="btn btn-primary float-right btn-sm"><i class="fas fa-plus"></i> {{__('Add Article')}}</a>
                    <a href="{{route('admin.articles.categories.index')}}" class="btn btn-info float-right btn-sm mr-2"><i class="fas fa-list"></i> {{__('Manage Categories')}}</a>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                      @foreach ($articles as $article)
                        <tr>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm" href="{{route('admin.articles.show', $article->id)}}">
                              <i class="fas fa-eye"></i>
                            </a>
                            <a class="btn btn-secondary btn-sm" href="{{route('admin.articles.edit', $article->id)}}">
                              <i class="fas fa-edit"></i>
                            </a>
                            <form class="deleteform d-inline-block" action="{{route('admin.articles.destroy', $article->id)}}" method="post">
                              @csrf
