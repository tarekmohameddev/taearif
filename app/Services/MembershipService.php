...
class MembershipService
{
    ...
    /**
     * Handle package upgrade - disable maintenance mode if user upgrades from free
     */
    public function handlePackageUpgrade(User $user, int $newPackageId): void
    {
        ...
        // Only disable maintenance mode if upgrading from free package to paid package
        if ($this->hasFreePackage($user) && $newPackageId !== self::FREE_PACKAGE_ID) {
            $this->disableMaintenanceMode($user);
            
            // Fire event for upgrade
            event(new UserUpgradedFromFree($user, $newPackage));
            
            Log::info("Disabled maintenance mode for user {$user->id} after package upgrade from free to {$newPackage->title} (ID: {$newPackageId}, Term: {$newPackage->term})");
        } else {
            Log::info("Package upgrade for user {$user->id} to {$newPackage->title} (ID: {$newPackageId}, Term: {$newPackage->term}) - no maintenance mode change needed");
        }
    }
