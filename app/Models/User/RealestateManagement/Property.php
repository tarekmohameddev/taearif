...
class Property extends Model
{
    ...
    public function updateProperty($requestData)
    {
        ...
        $pick = static function (string $field, $fallback = null) use ($requestData) {
            return array_key_exists($field, $requestData) ? $requestData[$field] : $fallback;
        };
        ...
        return $this->update([
            ...
            'video_image' => $pick('video_image', $this->video_image),
            'price' => $pick('price', $this->price),
            'pricePerMeter' => $pick('pricePerMeter', $this->pricePerMeter),
            'purpose' => $pick('purpose', $this->purpose),
            'listing_purpose' => $pick('listing_purpose', $this->listing_purpose),
            'unit_status' => $pick('unit_status', $this->unit_status),
            'publish_status' => $pick('publish_status', $this->publish_status),
            'property_type' => array_key_exists('property_type', $requestData)
                ? $requestData['property_type']
                : (array_key_exists('type', $requestData) ? $requestData['type'] : $this->property_type),
            'beds' => $pick('beds', $this->beds),
            'bath' => $pick('bath', $this->bath),
            'area' => $pick('area', $this->area),
            'size' => $pick('size', $this->size),
            'featured' => $pick('featured', $this->featured),
            'video_url' => $pick('video_url', $this->video_url),
            'virtual_tour' => $pick('virtual_tour', $this->virtual_tour),
            'status' => $pick('status', $this->status),
            'features' => $pick('features', $this->features),
            'latitude' => $pick('latitude', $this->latitude),
            'longitude' => $pick('longitude', $this->longitude),
            'category_id' => $pick('category_id', $this->category_id),
            'payment_method' => $pick('payment_method', $this->payment_method),
            'faqs' => $pick('faqs', $this->faqs),
            'building_id' => $pick('building_id', $this->building_id),
            'water_meter_number' => $pick('water_meter_number', $this->water_meter_number),
            'electricity_meter_number' => $pick('electricity_meter_number', $this->electricity_meter_number),
            'deed_number' => $pick('deed_number', $this->deed_number),
            'advertising_license' => $pick('advertising_license', $this->advertising_license),
            'owner_number' => $pick('owner_number', $this->owner_number),
            'reorder_featured' => $pick('reorder_featured', $this->reorder_featured),
            'reorder' => $pick('reorder', $this->reorder),
            'show_reservations' => $pick('show_reservations', $this->show_reservations),
            'completion_status' => $pick('completion_status', $this->completion_status),
            'missing_fields' => $pick('missing_fields', $this->missing_fields),
            'validation_errors' => $pick('validation_errors', $this->validation_errors),
            'completed_at' => $pick('completed_at', $this->completed_at),
