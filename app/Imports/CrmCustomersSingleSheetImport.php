...
class CrmCustomersSingleSheetImport implements OnEachRow, WithHeadingRow, WithValidation, SkipsOnFailure, SkipsOnError, WithLimit
{
    ...
    protected $employeeNameLookup = [];
    protected $employeeEmailLookup = [];
    ...
    public function __construct($userId, $limit = 1000)
    {
        ...
        // Get tenant ID for employee lookups
    ...
    protected function loadLookups(): void
    {
        ...
        if ($this->tenantId) {
            $employees = User::where('tenant_id', $this->tenantId)
    ...
    public function onRow(Row $row)
    {
        ...
        $responsibleEmployeeId = $this->resolveEmployee($row, $rowIndex);

        // Wrap entire row processing in transaction
        DB::transaction(function () use ($row, $rowIndex, $typeId, $priorityId, $stageId, $procedureId, $cityId, $districtId, $responsibleEmployeeId) {
            $customerData = [
                'user_id' => $this->userId,
                'name' => $row['name'] ?? null,
    ...
    /**
     * Resolve employee from responsible_employee_name, responsible_employee_email, or responsible_employee_id
     */
    protected function resolveEmployee(array $row, int $rowIndex): ?int
    {
        // First check if responsible_employee_id is provided directly
        if (!empty($row['responsible_employee_id']) && is_numeric($row['responsible_employee_id'])) {
            $employeeId = (int) $row['responsible_employee_id'];
            if ($this->tenantId) {
            ...
            throw new \Exception("Row {$rowIndex}: Invalid responsible_employee_id: {$row['responsible_employee_id']}. Employee does not exist or is not active for this tenant.");
        }

        // Check responsible_employee_email
        if (!empty($row['responsible_employee_email'])) {
            ...
            throw new \Exception("Row {$rowIndex}: Unknown responsible_employee_email: '{$row['responsible_employee_email']}'. Please check the employees sheet for valid employees.");
        }

        // Check responsible_employee_name
        if (!empty($row['responsible_employee_name'])) {
            ...
            throw new \Exception("Row {$rowIndex}: Unknown responsible_employee_name: '{$row['responsible_employee_name']}'. Please check the employees sheet for valid employees.");
        }
