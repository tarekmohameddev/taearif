...
class UpdateCustomerRequest extends BaseApiFormRequest
{
    ...
    public function rules()
    {
        ...
        $customerId = $customer instanceof \Illuminate\Database\Eloquent\Model ? $customer->id : $customer;
        ...
        return [
            ...
            'email' => [
                ...
                'nullable',
                'email',
                Rule::unique('api_customers', 'email')->where(fn ($q) => $q->where('user_id', $userId))->ignore($customerId),
            ],
            'phone_number' => [
                'sometimes',
                'string',
                'max:20',
                Rule::unique('api_customers', 'phone_number')->where(fn ($q) => $q->where('user_id', $userId))->ignore($customerId),
            ],
