...
class CrmCustomersTemplateExport implements WithMultipleSheets
{
    protected $userId;
    ...
    {
        return [
            new CrmCustomersTemplateMainSheetExport($this->userId),
            new CrmCustomersTemplateLookupSheetExport($this->userId),
            new CrmCustomersTemplateCityDistrictSheetExport(),
            new CrmCustomersTemplateEmployeesSheetExport($this->userId),
        ];
    }
}

class CrmCustomersTemplateMainSheetExport implements FromArray, WithHeadings, WithTitle, WithEvents
{
    protected $userId;
    ...
    public function __construct($userId)
    {
        ...
        if ($this->userId) {
            ...
            // Get tenant ID for employees
            $user = User::find($this->userId);
            if ($user) {
                $tenantId = $user->tenantOwnerId();
                $this->employeeCount = User::where('tenant_id', $tenantId)
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $this->addDropdown($sheet, 'K', 'Employees', 'A', $this->employeeCount);
            },
        ];
    }
...
class CrmCustomersTemplateLookupSheetExport implements FromCollection, WithHeadings, WithTitle, WithEvents
{
    protected $userId;
...
class CrmCustomersTemplateCityDistrictSheetExport implements FromCollection, WithHeadings, WithTitle, WithEvents
{
    public function collection()
    {
...
class CrmCustomersTemplateEmployeesSheetExport implements FromCollection, WithHeadings, WithTitle, WithEvents
{
    protected $userId;
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('1:1')->applyFromArray([
                    ...
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
                        'startColor' => ['rgb' => 'FF9800'], // Orange for employees
                    ],
                    'alignment' => [
                        'horizontal' => Alignment::HORIZONTAL_CENTER,
                        'vertical' => Alignment::VERTICAL_CENTER,
